...
<body>
	<!-- 新建分组 -->
	<div class="jumbotron" id="newPictureGroup" style="position: fixed; left:5%; top:5%; z-index: 2; padding: 20px; display: none;">
	   <div class="container">
	   		<div class="panel panel-default blog-panel">
			  <!-- Default panel contents -->
			  <div class="blog-panel-heading">添加图片分组</div>
			  	<form action="pictureAction!newPictureGroup" method="post" name="groupForm" id="newPictureGroupForm">
					  <!-- List group -->
					  <ul class="list-group">
					    <li class="list-group-item">
							<div class="input-group">
							  <span class="input-group-addon" id="basic-addon1">分组名称:</span>
							  <input type="text" class="form-control" placeholder="group-name" name="pictureGroupForm.groupName" aria-describedby="basic-addon1">
							</div>
						</li>
					    ...
					    <li class="list-group-item" style="padding-top: 20px;">
					    	...
					    	<br><br>
							<textarea rows="10" style="width:100%" name="pictureGroupForm.groupDescrib" placeholder="请输入分组描述（不要超过100字符）"></textarea>
					    </li>
					    
					    <li class="list-group-item" style="padding-top: 20px;">
					    	<center>
					    		<button type="button" class="btn btn-success" onclick="$('#newPictureGroupForm').submit();"><span class="glyphicon glyphicon-floppy-saved" aria-hidden="true"></span>&nbsp;创建</button>
					    		<button type="button" class="btn btn-warning" style="margin-left: 50px;"  onclick="$('#newPictureGroup')[0].style.display='none';"><span class="glyphicon glyphicon-floppy-remove" aria-hidden="true"></span>&nbsp;取消</button>
					    	</center>
					    </li>
					  </ul>
			  	</form>
	...
	<!-- 更改分组面板 -->
	<div class="jumbotron" id="renamePictureGroup" style="position: fixed; left:5%; top:5%; z-index: 2; padding: 20px; display: none;">
	   <div class="container">
	   		<div class="panel panel-default blog-panel">
			  <!-- Default panel contents -->
			  <div class="blog-panel-heading">更改图片分组</div>
			  	<form action="pictureAction!pictureRenameGroup" method="post" name="groupForm" id="renamePictureGroupForm">
			  		  <input type="hidden" name="pictureGroupForm.orgGroupName" id="renamePictureGroupForm_preGroupName" value=""/>
					  <!-- List group -->
					  <ul class="list-group">
					    <li class="list-group-item">
							<div class="input-group">
							  <span class="input-group-addon" id="basic-addon1">分组名称:</span>
							  <input type="text" class="form-control" placeholder="group-name" name="pictureGroupForm.groupName" id="renamePictureGroupForm_groupName" aria-describedby="basic-addon1">
							</div>
						</li>
					    ...
					    <li class="list-group-item" style="padding-top: 20px;">
					    	<span class="label label-default blog-label-1">分组描述:</span>
					    	<br><br>
							<textarea rows="10" style="width:100%" name="pictureGroupForm.groupDescrib" id="renamePictureGroupForm_groupDescrib" placeholder="请输入分组描述（不要超过100字符）"></textarea>
					    </li>
					    
					    <li class="list-group-item" style="padding-top: 20px;">
					    	<center>
					    		<button type="button" class="btn btn-success" onclick="$('#renamePictureGroupForm').submit();"><span class="glyphicon glyphicon-floppy-saved" aria-hidden="true"></span>&nbsp;更改</button>
					    		<button type="button" class="btn btn-warning" style="margin-left: 50px;"  onclick="$('#renamePictureGroup')[0].style.display='none';"><span class="glyphicon glyphicon-floppy-remove" aria-hidden="true"></span>&nbsp;取消</button>
					    	</center>
					    </li>
					  </ul>
			  	</form>
			      ...
			      <ul class="nav navbar-nav navbar-right blog_nav">
			        <li class="dropdown">
			          ...
			          <ul class="dropdown-menu blog-menu">
			            <li><a href="javascript:void(0)" onclick="$('#newPictureGroup')[0].style.display='';">新建分组</a></li>
				  	...
				  	<div class="blog-panel-heading">
			       	...
			       	<div class="panel-body">
			       	   <table class="table table-hover">
					       ...
					       <tbody>
							  <c:forEach var="group" items="${session.pictureGroupJspGetAllGroups}" varStatus="status">
						  		<tr style="cursor: pointer;">
						       		<td scope="row" style="vertical-align: middle;"  onclick="location.href='pictureAction!pictureJspGetOneGroup?groupId=${group.groupId}';">
									    <h1>${status.count}<h1>
								    </td>
							 		<td style="vertical-align: middle;"  onclick="location.href='pictureAction!pictureJspGetOneGroup?groupId=${group.groupId}';">
							 			...
							 			<c:choose>
							 				...
							 				<c:otherwise>
							 					<img src="./image/messageImages/404.png" alt="${group.groupName}" title="${group.groupName}" height="50" width="50">
							 				</c:otherwise>
							 			</c:choose>
							 			</a>
								    </td>
									...
									<td style="vertical-align: middle; display: none;" name="hjzgg_delete" id="hjzgg_deleteId${status.count}">
							 			<a class="btn btn-info" href="javascript:void(0)" style="float:right;" role="button" onclick="myDeletePictureGroup('hjzgg_deleteId${status.count}', '${group.groupId}');">删除</a>
							 		</td>
							 		<td style="vertical-align: middle; display: none;" name="hjzgg_rename" id="hjzgg_renameId${status.count}">
							 			<a class="btn btn-info" href="javascript:void(0)" style="float:right;" role="button" onclick="$('#renamePictureGroupForm_preGroupName')[0].value='${group.groupName}'; $('#renamePictureGroupForm_groupName')[0].value='${group.groupName}'; $('#renamePictureGroupForm_groupDescrib')[0].value='${group.groupDescrib}'; $('#renamePictureGroup')[0].style.display='';">更改分组</a>
							 		</td>
								</tr>
							  </c:forEach>
					       </tbody>
...
<script type="text/javascript">
	...
	function myDeletePictureGroup(operationId, groupId){
		$.ajax({
		    type:"post",
		    url: "pictureAction!deletePictureGrouop",
