...
<body onload="$('#commentsList')[0].scrollTop=$('#commentsList')[0].scrollHeight;">
		    ...
		    <div class="col-xs-12 col-sm-6 col-md-8">
		    	<div class="panel panel-default blog-panel">
				  <div class="blog-panel-heading">																		
				    <h3 class="panel-title"><span class="glyphicon glyphicon-send blog-icon" aria-hidden="true"></span>${session.articleJspGetOneArticle.articleTitle}</h3>
				  </div>
				  <div class="panel-body" id="myArticleDivId">
				  		${session.articleJspGetOneArticle.articleContent}
				  </div>
				  <div class="blog-postDesc">posted @ ${article.articleBuildTime} 胡峻峥 阅读(${session.articleJspGetOneArticle.articleReadingCount}) 评论(${fn:length(session.articleJspGetOneArticle.comments)})  <a href="blogAction!updateEditJpsGetOneArticle?articleId=${session.articleJspGetOneArticle.articleId}" rel="nofollow">编辑</a></div>
				</div>
		    	<b>随笔分类:</b>
		    	<c:forEach var="group" items="${session.articleJspGetOneArticle.groups}">
				    <label style="margin-left: 5px; margin-right: 5px;">
				      	<a href="blogAction!groupJspGetOneGroup?blogGroupId=${group.groupId}">${group.groupName}</a>
				    </label>
		    	</c:forEach>
		    	...
		    	<form action="#" method="post" id="newArticleCommentForm">
		    		<input type="hidden" name="blogArticleCommentForm.blogArticleId" value="${session.articleJspGetOneArticle.articleId}"/>
			    	<div class="panel panel-default blog-panel">
						  <div class="blog-panel-heading">																		
						    <h3 class="panel-title"><span class="glyphicon glyphicon-globe" aria-hidden="true"></span>&nbsp;&nbsp;欢迎讨论</h3>
						  </div>
						  <div class="input-group input-group-lg" style="margin-left: 12px; margin-right: 10px; margin-top: 10px;">
							  	...
							  	<input type="text" class="form-control" name="blogArticleCommentForm.commentPeopleContact" placeholder="your e-mail" aria-describedby="sizing-addon1">
						  </div>
						  <div class="panel-body">
...
<script type="text/javascript">
	uParse('.myArticleDivId', {rootPath: './'});
	
	function formAppendSubmit(){
		var myform=$('#newArticleCommentForm'); 
		var tmpInput=$("<input type='text' name='blogArticleCommentForm.blogArticleCommentContent'/>");
		if(!hasContent()){
			alert("请输入内容！");
			return ;
		}
		...
		$.ajax({
		    type:"post",
		    url: "blogAction!newBlogArticleComment",
